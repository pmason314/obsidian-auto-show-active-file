import { FileView, Plugin, WorkspaceLeaf, Notice } from "obsidian";
...
export default class MyPlugin extends Plugin {
	...
	private is_file_explorer_open() {
		...
		workspace.iterateAllLeaves((leaf) => {
			if (
				...
				window.getComputedStyle(leaf.containerEl, null).display !==
					"none"
			) {
				is_open = true;
			}
	...
	private async reveal() {
		...
		if (this.current_open_file_leaf) {
			this.app.workspace.setActiveLeaf(this.current_open_file_leaf, {
				focus: true,
			});
		}
	}

	private isFocusOnPageTitle(): boolean {
		const activeElement = document.activeElement;
		if (activeElement) {
			return activeElement.className === "inline-title";
		}
		return false;
	}
	...
	onload() {
		...
		this.app.workspace.on("file-open", async (file) => {
			...
			if (is_file_explorer_open_now && !this.isFocusOnPageTitle()) {
				this.reveal();
			}
		});
		...
		this.app.workspace.on("active-leaf-change", async (leaf) => {
			...
			if (this.is_file_explorer(leaf)) {
				if (
					is_file_explorer_open_now &&
					!this.is_file_explorer_open_previously
				) {
					this.reveal();
				}
			}
